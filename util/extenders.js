...
Interaction.prototype.sendE = function(string, options = {}){
    ...
    if(options.edit){
      ...
      return this.followUp({embeds: [embed], components: options.row ? [options.row] : []})
    }
  }
...
Interaction.prototype.success = function(string){
  let rep = [
    `You can join My support server at ${this.client.config.support}`,
    `Want to invite me ? \nHere you go ${this.client.config.invite}`,
    "** **",
    "** **",
    "** **",
    "** **"
  ]
  this.followUp({content: `${rep[Math.floor(Math.random() * rep.length)]}`, embeds: [
    new MessageEmbed()
  .setAuthor({ name: this.user.username, iconURL: this.user.displayAvatarURL() })
  .setDescription(`${emoji.yes} | ${string}`)
    .setColor("GREEN")
